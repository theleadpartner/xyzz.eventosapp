...
add_action('admin_post_eventosapp_task_queue_action', function() {
    ...
    if ( $action === 'delete_terminal' ) {
        ...
        if ( $requested_status !== '' ) {
            ...
            $deleted = eventosapp_task_queue_delete_terminal('completed')
                     + eventosapp_task_queue_delete_terminal('cancelled');
        }
        eventosapp_task_queue_admin_redirect('Se eliminaron ' . absint($deleted) . ' tareas finalizadas o canceladas.', true);
    }
    ...
    switch ( $action ) {
        ...
        case 'reschedule':
            ...
            try {
                $local = new DateTimeImmutable(trim($date . ' ' . $time), $timezone);
                $utc = $local->setTimezone(new DateTimeZone('UTC'))->format('Y-m-d H:i:s');
                $ok = eventosapp_task_queue_reschedule($task_id, $utc);
            } catch (Throwable $e) {
                $ok = false;
            }
            $message = $ok ? 'Tarea reprogramada.' : 'La fecha u hora no son válidas para reprogramar.';
            break;
        case 'delete':
            $ok = eventosapp_task_queue_delete($task_id);
            $message = $ok ? 'Tarea eliminada.' : 'Solo se pueden eliminar tareas finalizadas, canceladas o con error.';
...
function eventosapp_task_queue_admin_progress($task) {
    ...
    if ( $total < 1 ) return in_array($task['status'] ?? '', eventosapp_task_queue_terminal_statuses(), true) ? 100 : 0;
...
function eventosapp_task_queue_admin_local_datetime($utc, $format = 'd/m/Y H:i:s') {
    if ( ! $utc ) return '—';
    try {
...
function eventosapp_task_queue_render_task_list() {
    ...
    <form method="get" class="evapp-queue-filters">
        ...
        <select name="status"><option value="">Todos los estados</option><?php foreach($statuses as $key=>$label): ?><option value="<?php echo esc_attr($key); ?>" <?php selected($filters['status'],$key); ?>><?php echo esc_html($label); ?></option><?php endforeach; ?></select>
    ...
    <div class="evapp-queue-toolbar">
        ...
        <div>
            <?php
            echo eventosapp_task_queue_admin_action_form(['id'=>0], 'delete_terminal', 'Borrar finalizadas y canceladas', 'button', '<input type="hidden" name="terminal_status" value="">');
            ?>
        </div>
    </div>
    ...
    <?php if ( empty($result['items']) ): ?><tr><td colspan="8"><div class="evapp-queue-empty">No hay tareas con estos filtros.</div></td></tr><?php else: foreach($result['items'] as $task):
        $progress = eventosapp_task_queue_admin_progress($task);
        ...
        <tr data-task-id="<?php echo esc_attr($task['id']); ?>">
            <td><strong><a href="<?php echo esc_url(eventosapp_task_queue_task_url($task['id'])); ?>"><?php echo esc_html($task['title']); ?></a></strong><br><code>#<?php echo esc_html($task['id']); ?> · <?php echo esc_html($task['uuid']); ?></code><br><small>Creada: <?php echo esc_html(eventosapp_task_queue_admin_local_datetime($task['created_at'])); ?></small></td>
            ...
            <td><?php echo $task['scheduled_at'] ? '<strong>'.esc_html(eventosapp_task_queue_admin_local_datetime($task['scheduled_at'])).'</strong>' : 'Inmediata'; ?><br><small>Próximo turno: <?php echo esc_html(eventosapp_task_queue_admin_local_datetime($task['next_run_at'])); ?></small></td>
            <td class="evapp-queue-actions"><a class="button button-small" href="<?php echo esc_url(eventosapp_task_queue_task_url($task['id'])); ?>">Gestionar</a><?php if($task['status']==='paused') echo eventosapp_task_queue_admin_action_form($task,'resume','Reanudar','button button-small'); elseif(!in_array($task['status'],eventosapp_task_queue_terminal_statuses(),true)) echo eventosapp_task_queue_admin_action_form($task,'pause','Pausar','button button-small'); ?><?php if(!in_array($task['status'],eventosapp_task_queue_terminal_statuses(),true)) echo eventosapp_task_queue_admin_action_form($task,'cancel','Cancelar','button button-small evapp-button-danger'); ?></td>
        </tr>
...
function eventosapp_task_queue_render_task_detail($task_id) {
    ...
    <div class="evapp-queue-detail" id="evappQueueTask" data-task-id="<?php echo esc_attr($task_id); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce('eventosapp_task_queue_status')); ?>">
        ...
        <aside class="evapp-queue-detail-side">
            ...
            <div class="evapp-queue-card"><h3>Datos</h3><dl class="evapp-queue-dl"><dt>Evento</dt><dd><?php echo $task['event_id']?esc_html(get_the_title($task['event_id'])):'—'; ?></dd><dt>Cliente</dt><dd><?php echo $task['client_id']?esc_html(get_the_title($task['client_id'])):'—'; ?></dd><dt>Canal</dt><dd><?php echo esc_html($task['channel'] ?: '—'); ?></dd><dt>Tipo</dt><dd><?php echo esc_html(eventosapp_task_queue_admin_task_label($task['task_type'])); ?></dd><dt>Programada</dt><dd><?php echo esc_html(eventosapp_task_queue_admin_local_datetime($task['scheduled_at'])); ?></dd><dt>Próximo turno</dt><dd><?php echo esc_html(eventosapp_task_queue_admin_local_datetime($task['next_run_at'])); ?></dd></dl></div>
        </aside>
    </div>
    <script>
    jQuery(function($){
