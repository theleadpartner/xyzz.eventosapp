...
function eventosapp_task_queue_render_task_list() {
    ...
    <form method="get" class="evapp-queue-filters">
        ...
        <select name="channel"><option value="">Todos los canales</option><?php foreach(['tickets'=>'Tickets / Importación','email'=>'Correo','whatsapp'=>'WhatsApp','flow'=>'Flow','mixed'=>'Mixto','email+whatsapp'=>'Correo + WhatsApp'] as $key=>$label): ?><option value="<?php echo esc_attr($key); ?>" <?php selected($filters['channel'],$key); ?>><?php echo esc_html($label); ?></option><?php endforeach; ?></select>
