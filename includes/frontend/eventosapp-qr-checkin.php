...
// Busca por meta 'eventosapp_ticketID' (ID público opaco) o 'eventosapp_ticket_preprintedID' (si el evento lo usa),
// valida que pertenezca al evento y marca el día actual (en la TZ del evento) como 'checked_in'.
//
...
add_action('wp_ajax_eventosapp_qr_sesion_lookup', function(){
    ...
    if ( ! $ticket_post_id ) {
        wp_send_json_error(['error'=> $use_preprinted ? 'Ticket no encontrado para este evento (ID preimpreso).' : 'Ticket no encontrado']);
    }
    // Seguridad extra
